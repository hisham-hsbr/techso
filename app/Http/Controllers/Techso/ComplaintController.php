...
class ComplaintController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $complaint->local_name = $request->local_name;

        if ($request->default == 0) {
            $complaint->default == 0;
        } else {
            $default = (DB::table('complaints')->where('default', 1)->first())->id;

            $update_default = Complaint::find($default);
            $update_default->default = null;
            $update_default->update();
        }

        $complaint->default = $request->default;
